...
export class DocumentService {
	...
	async ingestDocument(input: {
		...
		const document = await prisma.knowledgeDocument.create({
			data: {
				type: input.type, // schema allows string; shared narrows union at runtime
				title: input.title,
				content: content,
				sourceUrl: input.url,
		...
		await embeddingsService.generateEmbeddings(
			chunks.map(chunk => chunk.content)
		);

		// Save chunks (skip embedding write for now to satisfy Prisma Unsupported type)
		for (let i = 0; i < chunks.length; i++) {
			await prisma.knowledgeDocument.create({
				data: {
					...
					metadata: {
						...(chunks[i].metadata as any),
						parentTitle: document.title
					} as any,
					// embedding: embeddings[i] as any, // temporarily disabled
					chunkIndex: i,
